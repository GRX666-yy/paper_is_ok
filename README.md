...
7. **生成报告** — 统一放入 `查重报告\` 文件夹：有 LaTeX 环境输出 `查重报告.tex` 并编译为 `查重报告.pdf`；**无 LaTeX 环境（如只上传 PDF 的用户）输出 `查重报告.docx`（Word 版，红/橙/黄底纹高亮）并自动转换为 `查重报告.pdf`**；若编译时发现 LaTeX 环境不完整（缺宏包），会**先询问您是否补全环境**，愿意则引导安装后重编译，不愿意则自动切换 Word 模式；全文重现并高亮，无疑似不标记，并随附**降重指南**（基于知网降重原理的 `references/reduce-duplication.md`）。
...
| 轻微疑似 | 0.2 | 黄色 | 连续重合 6–9 字符 / 观点高度相近 / 规范直接引用 |
| 中度疑似 | 0.3 | 橙色 | 连续重合 10–17 字符 / 单句同构仅换措辞 |
| 严重疑似 | 0.5 | 红色 | 连续重合 ≥18 字符 / 整段同构复写 |
