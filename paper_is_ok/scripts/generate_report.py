...
RUBRIC_RECAP = [
    ("严重疑似（红）", "0.5",
     "连续重合不低于18字符；或整段同构复写/拼接复写（R1a--R1c）"),
    ("中度疑似（橙）", "0.3",
     "连续重合10--17字符；或单句同构、短句连续命中（R2a--R2c）"),
    ("轻微疑似（黄）", "0.2",
     "连续重合6--9字符；或观点高度相近（R3a--R3b）；规范引用（R3c）标记"
     "quote=true，不计入查重率"),
    ("无疑似（不标记）", "0",
...
def build_report(meta_title, blocks, resolved, stats, rate, rate_no_quote,
    ...
    L.append(r"\noindent \textbf{结果说明}：本查重率仅基于本次可获取并已比对的"
             ...
             r"知网实际连续匹配阈值约为 13--15 字符，与本报告的严重疑似"
             r"阈值（18 字符）接近但本报告仍略宽松。\textbf{仅供参考，"
...
def build_docx(meta_title, blocks, resolved, stats, rate, rate_no_quote,
    ...
    para("结果说明：本查重率仅基于本次可获取并已比对的文献子集；付费跳过的文献未参与"
         "比对，实际重复率只可能更高。知网实际连续匹配阈值约为 13–15 字符，与本报告"
         "的严重疑似阈值（18 字符）接近但本报告仍略宽松。仅供参考，不等同于知网/"
         "iThenticate 等官方查重报告。")
